...
<div class="card">
    ...
    <form action="{{ route('pelanggaran.store') }}" method="POST">
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <select name="kelompok" id="kelompok" class="form-control select2 @error('kelompok') is-invalid @enderror" required>
                    ...
                    @php
                        $kelompoks = $jenisPelanggarans->pluck('kelompok')->unique()->sort()->values();
                    @endphp
            ...
            <div class="form-group">
                ...
                <select name="jenis_pelanggaran_id" id="jenis_pelanggaran_id" class="form-control select2 @error('jenis_pelanggaran_id') is-invalid @enderror" required>
                    <option value="">Pilih Kategori Pelanggaran Terlebih Dahulu</option>
                    @foreach($jenisPelanggarans as $jenis)
                        <option value="{{ $jenis->id }}" data-poin="{{ $jenis->poin }}" data-kelompok="{{ $jenis->kelompok }}" {{ old('jenis_pelanggaran_id') == $jenis->id ? 'selected' : '' }} style="display:none;">
                            {{ $jenis->nama_pelanggaran }} - {{ $jenis->poin }} poin
                        </option>
                    @endforeach
                </select>
                @error('jenis_pelanggaran_id')<span class="invalid-feedback">{{ $message }}</span>@enderror
            </div>
        ...
        <div class="card-footer">
            ...
            <a href="{{ route('pelanggaran.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Kembali</a>
        </div>
    </form>
</div>
@endsection
...
$(document).ready(function() {
    ...
    $('#kelompok').change(function() {
        ...
        var $jenisSelect = $('#jenis_pelanggaran_id');
        
        $jenisSelect.find('option').each(function() {
            var optKelompok = $(this).data('kelompok');
            if ($(this).val() === '') {
                $(this).show();
            } else if (kelompok === '' || optKelompok === kelompok) {
                $(this).show();
            } else {
                $(this).hide();
            }
